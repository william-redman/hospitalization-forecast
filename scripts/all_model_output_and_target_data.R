...
for (year in 2019:2024) {
  ...
  file_path <- file.path(season_folder, "hospitalization-data.csv")
  ...
  if (file.exists(file_path)) {
    ...
    data <- read.csv('target-data/season_2024_2025/hospitalization-data.csv', stringsAsFactors = FALSE)
    ...
    # Append the data to the list
    all_data[[length(all_data) + 1]] <- data
    print("File found at: target-data/season_2024_2025/hospitalization-data.csv")
...
write.csv(final_data, "auxiliary-data/concatenated_hospitalization_data.csv", row.names = FALSE)
...
all_model_data <- all_model_data %>%
  mutate(values = ifelse(values < 0, 0, ceiling(values)))
